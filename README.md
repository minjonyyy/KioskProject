...
### ✅ CartItem.java
* MenuItem 클래스와는 별도로 장바구니 아이템은 따로 관리해주기 위한 클래스이다.
* 메뉴 이름, 가격, 수량으로 필드가 구성되어있음
   * Cart클래스의 `addMenuToCart()`메서드를 통해 이름, 가격은 MenuItem에서 받아온다.
   * 수량은 맨 처음 장바구니 추가 시에는 기본 1개로 설정해두고, 이후 동일메뉴 추가 시에는 수량이 +1씩 늘어난다.
